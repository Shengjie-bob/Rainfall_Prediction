...
def FakeBagging(path, dirlist, test_x, test_y, id_num):
    num = len(dirlist)
    ...
    # 绘图
    xmark = [ele.split('.')[0] for ele in dirlist]
    xmark = [ele.split('_')[0] for ele in dirlist]
    for i,ele in enumerate(xmark):
        if ele == 'train':
            xmark[i] = dirlist[i].split('_')[1]
    ...
    plt.grid();plt.savefig('output/bagging_rmse_meric_%d'%id_num,bbox_inches='tight',dpi=500)
    ...
    plt.plot(r2_list, '^-',c='r',label = 'r2_score');plt.legend();plt.xticks(range(num+1),xmark,rotation=45)
    plt.plot(var_list,'D-',c='darkorchid', label = 'var');plt.legend();plt.xticks(range(num+1),xmark,rotation=45)
    plt.grid();plt.savefig('output/bagging_4_merics_%d'%id_num,bbox_inches='tight',dpi=500)
    plt.show()
...
def stackingUseGBRT(path, dirlist, est_g, dep_g, lr_g, train_x, train_y, test_x, test_y, id_num):
    num = len(dirlist)  # 基学习期个数
    rmse_list, mae_list, mdae_list, r2_list, var_list = [], [], [], [], []
    for ele in dirlist:
        a = ele.split('.')
    ...
    plt.figure();plt.plot(rmse_list, 'o-',c='salmon', label = 'rmse');plt.legend();plt.xticks(range(num+1),xmark,rotation=45)
    plt.grid();plt.savefig('output/stacking_rmse_meric_%d'%id_num,bbox_inches='tight',dpi=500)
    plt.figure();plt.plot(mae_list, '*-',c='limegreen', label = 'mae');plt.legend();plt.xticks(range(num+1),xmark,rotation=45)
    plt.plot(mdae_list,'v-',c='blue',label = 'mdae');plt.legend();plt.xticks(range(num+1),xmark,rotation=45)
    plt.plot(r2_list, '^-',c='r',label = 'r2_score');plt.legend();plt.xticks(range(num+1),xmark,rotation=45)
    plt.plot(var_list, 'D-',c='darkorchid', label = 'var');plt.legend();plt.xticks(range(num+1),xmark,rotation=45)
    plt.grid();plt.savefig('output/stacking_4_merics_%d'%id_num,bbox_inches='tight',dpi=500)
    plt.show()
...
if __name__ == '__main__':
    pathnow = os.getcwd()
    path = '{}/MODEL_NO'.format(pathnow)
    dirlist = os.listdir(path)  # 加载保存在MODEL文件夹里的所有.pth和.m模型
    # dirlist.remove('.DS_Store')

    # 加载测试数据集
    id_num = 312
    train_x = np.loadtxt("{}/train_test_hour/train{}.txt".format(pathnow, id_num))
    train_y = np.loadtxt("{}/train_test_hour/train_label{}.txt".format(pathnow, id_num))
    test_x = np.loadtxt("{}/train_test_hour/test{}.txt".format(pathnow, id_num))
    test_y = np.loadtxt("{}/train_test_hour/test_label{}.txt".format(pathnow, id_num))
