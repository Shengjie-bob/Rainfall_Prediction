...
from att_seq2seq_module import Seq2Seq
...
parser.add_argument('--input-dim', type=int, default=7, help='num of hour')
parser.add_argument('--seq-len', type=int, default=3, help='num of parameter each hour')
...
model = Seq2Seq(num,input_dim,seq_len,output_dim=1,hidden_size=128,dropout=0.1,learning_rate=learning_rate,batch_size=batch_size,device_pu=device)

eval_best=model.fit(train_feature,train_rain,num_epoches=num_epoches,shuffle=False)
print("best val:"+str(eval_best)+'\n')

y_seq2seq,y_attention=model.predict(test_feature,test_rain)
...
print('ATTSEQ2SEQ_rmse: %r' % rmse,
      'ATTSEQ2SEQ_mae: %r' % mae,
      'ATTSEQ2SEQ_mdae: %r' % mdae,
      'ATTSEQ2SEQ_r2: %r' % r2,
      'ATTSEQ2SEQ_var: %r' % var)
