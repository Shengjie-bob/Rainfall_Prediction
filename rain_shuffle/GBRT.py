...
# for lr in n_lr:
#     # 模型训练，使用GBDT算法
#     gbr = GradientBoostingRegressor(n_estimators=est_g, max_depth=dep_g, min_samples_split=3, learning_rate=lr)
#     gbr.fit(x_train, y_train.ravel())
#     y_pre_train = gbr.predict(x_train)
#     y_pre_test = gbr.predict(x_test)
...
# for est in n_est:
#     # 模型训练，使用GBDT算法
#     gbr = GradientBoostingRegressor(n_estimators=est, max_depth=dep_g, min_samples_split=3, learning_rate=lr_g)
#     gbr.fit(x_train, y_train.ravel())
#     y_pre_train = gbr.predict(x_train)
#     y_pre_test = gbr.predict(x_test)
#     error_1 = mean_squared_error(y_train, y_pre_train)
...
# for dep in n_dep:
#     # 模型训练，使用GBDT算法
#     gbr = GradientBoostingRegressor(n_estimators=est_g, max_depth=dep, min_samples_split=3, learning_rate=lr_g)
#     gbr.fit(x_train, y_train.ravel())
#     y_pre_train = gbr.predict(x_train)
#     y_pre_test = gbr.predict(x_test)
#     error_1 = mean_squared_error(y_train, y_pre_train)
...
gbr.fit(x_train, y_train.ravel())
joblib.dump(gbr, '{}/train_GBRT_{}.m'.format(path,num))   # 保存模型
clf=joblib.load('{}/train_GBRT_{}.m'.format(path,num))
y_gbrt = clf.predict(test_feature)
error = mean_squared_error(test_rain, y_gbrt)
print('Model Test MSE: %.3f' % error)

plt.plot(test_rain, 'b', label='real')
plt.plot(y_gbrt, 'r', label='prediction',alpha=0.3)
plt.legend(loc='best')
plt.show()

rmse, mae, mdae,r2,var = evaluation(test_rain, y_gbrt)

print('GBRT_rmse: %r' % rmse,
      'GBRT_mae: %r' % mae,
      'GBRT_mdae: %r' % mdae,
      'GBRT_r2: %r' % r2,
      'GBRT_var: %r' % var)

f = open('{}/test_GBRT_{}.txt'.format(path,num), 'w+')
f.write('GBRT_rmse: %r ' % rmse +
      'GBRT_mae: %r ' % mae +
      'GBRT_mdae: %r ' % mdae +
      'GBRT_r2: %r ' % r2 +
      'GBRT_var: %r ' % var)
